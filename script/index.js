...
const loadFood = (id) => {
  ...
  fetch(url)
    .then(res => res.json())
    .then(data => DisplayloadFood(data.plants)
  )
  
}

const DisplayloadFood = (plants) => {
...
const DisplayloadCatagory = (categories) => {
  ...
  for (let categorie of categories) {
    ...
    div.innerHTML =
      `
<button onclick="loadFood(${categorie.id})" class="btn btn-wide text-2xl font-bold hover:bg-green-500 rounded bg-[#15803D] text-white px-4 py-2">${categorie.category_name}</button>
